...
function wqnew_entry_callback_function() {
  global $wpdb;
  $wpdb->get_row( "SELECT * FROM `wp_crud` WHERE `name` = '".$_POST['kc_name']."' AND `email` = '".$_POST['kc_email']."' ORDER BY `id` DESC" );
  if($wpdb->num_rows < 1) {
    $wpdb->insert("wp_crud", array(
      "name" => $_POST['kc_name'],
      "email" => $_POST['kc_email'],
      "created_at" => time(),
      "updated_at" => time()
    ));
...
function wqedit_entry_callback_function() {
  global $wpdb;
  $wpdb->get_row( "SELECT * FROM `wp_crud` WHERE `name` = '".$_POST['kc_name']."' AND `email` = '".$_POST['kc_email']."' AND `id`!='".$_POST['wqentryid']."' ORDER BY `id` DESC" );
  if($wpdb->num_rows < 1) {
